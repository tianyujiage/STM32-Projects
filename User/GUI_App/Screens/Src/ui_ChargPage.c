...
static void ChargPage_timer_cb(lv_timer_t * timer)    //删除定时器
	//timer主要用于刷当前页面的数据，所以不在当前页面时需要删除掉。
{
    if(Page_Get_NowPage()->page_obj == &ui_ChargPage)//是否为充电页面
		{
      ...
      HWInterface.RealTimeClock.GetTimeDate(&DateTime);
			...
			if(ui_TimeMinuteValue != DateTime.Minutes)  //更新分钟
			{
				ui_TimeMinuteValue = DateTime.Minutes;  //把实时分钟赋给缓存分钟 
				sprintf(value_strbuf,"%02d",ui_TimeMinuteValue);
				lv_label_set_text(ui_ChargPagebMinLabel, value_strbuf);
			}

			if(ui_TimeHourValue != DateTime.Hours)  //更新小时
			{
			...
			ui_BatArcValue = HWInterface.Power.BatCalculate();  //获取电池电量
			if(ui_BatArcValue>0 && ui_BatArcValue<=100)  //电池电量刷新
			{
