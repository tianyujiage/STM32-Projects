...
static uint8_t page_stack_push(PageStack_t* stack, Page_t* page) {  //入栈函数
    if (stack->top >= MAX_DEPTH)
		return -1;//若栈已满返回-1
    stack->pages[stack->top++] = page;
	return 0;//若栈未满将page存入top指向的位置然后top自增，并返回0
}

static uint8_t page_stack_pop(PageStack_t* stack) {  //出栈函数
    if (stack->top <= 0)
		return -1;  //若栈为空则返回-1
    stack->pages[--stack->top]->deinit();
    return 0;  //若栈不空top自减，并调用deinit()反初始化函数,返回0
}
...
/**
 * back to bottom page home page到最底部的Home界面
 *
 * @param NULL
 * @return NULL
 */
...
/**
 * Load a new page to stack top加载新的界面
 *
...
/**
 * initialize the page manager & start home page初始化页面管理器并启动主页
 *
 * @param NULL
 * @return NULL
 */
