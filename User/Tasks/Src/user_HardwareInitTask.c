...
void HardwareInitTask(void *argument)
{
	while(1)
	{
    vTaskSuspendAll();  //挂起所有任务，防止初始化过程中被其他任务打断
    ...
    if(!HWInterface.Ecompass.ConnectionError)
      HWInterface.Ecompass.Sleep();  //若电子罗盘初始化，它进入睡眠模式
    ...
    if(!EEPROM_Check())
    {
      ...
      if((recbuf[0]!=0 && recbuf[0]!=1) || (recbuf[1]!=0 && recbuf[1]!=1))//若读取消息无效
      {
      ...
      SettingGet(recbuf,0x20,3);  //读取步数信息
